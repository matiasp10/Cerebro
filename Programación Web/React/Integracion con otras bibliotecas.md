...
## Integración con los plugins de manipulación del DOM
...
Adjuntaremos un [ref](https://es.reactjs.org/docs/refs-and-the-dom.html) al elemento DOM raíz. Dentro de `componentDidMount`, obtendremos una referencia a él para que podamos pasarlo al plugin jQuery.
...
Lo implementaremos como un [componente no controlado](https://es.reactjs.org/docs/uncontrolled-components.html) por simplicidad.
...
## Integración con otras bibliotecas de vista

React puede integrarse en otras aplicaciones gracias a la flexibilidad de [`createRoot()`](https://es.reactjs.org/docs/react-dom-client.html#createRoot).
...
Desde aquí, puedes comenzar a mover más lógica al componente y comenzar a adoptar prácticas de React más comunes. Por ejemplo, en los componentes es mejor no confiar en las ID porque el mismo componente se puede representar varias veces. En su lugar, usaremos el [sistema de eventos de React](https://es.reactjs.org/docs/handling-events.html) y registraremos el controlador de clic directamente en el elemento React `<button>`:
...
A continuación, crearemos una vista de Backbone llamada `ParagraphView`. Anulará la función `render()` de Backbone para renderizar un componente React `<Paragraph>` en el elemento DOM proporcionado por Backbone (`this.el`). Aquí también estamos usando [`ReactDOM.createRoot()`](https://es.reactjs.org/docs/react-dom-client.html#createroot):
...
Cuando se elimina un componente _desde dentro de_ un árbol de React, la limpieza se realiza automáticamente, pero como estamos eliminando todo el árbol a mano, debemos llamar a este método.

## Integración con capas de modelo

Aunque generalmente se recomienda usar un flujo de datos unidireccional como [el estado de React](https://es.reactjs.org/docs/lifting-state-up.html), [Flux](https://facebook.github.io/flux/), o [Redux](https://redux.js.org/), los componentes React pueden usar una capa modelo de otros frameworks.

### Usando Modelos de Backbone en Componentes de React
...
### Extracción de datos de modelos Backbone
